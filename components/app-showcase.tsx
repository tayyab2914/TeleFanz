...
export function AppShowcase() {
  return (
    <section id="for-users" className="py-20 sm:py-28 lg:py-36 overflow-hidden relative">
      ...
      {/* Pink accent glow */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-pink-600/20 rounded-full blur-[150px]" />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center max-w-3xl mx-auto mb-16 sm:mb-20 lg:mb-24">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-pink-500/10 border border-pink-500/20 mb-6 backdrop-blur-sm">
            <span className="text-sm font-medium text-pink-300">For Users</span>
          </div>
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-tight">
            Your Personalized <span className="text-gradient-pink">Social Experience</span>
          </h2>
          ...
          {showcaseItems.map((item, index) => (
            <div key={index} className="group">
              <div className="relative overflow-hidden rounded-3xl p-2 sm:p-3">
                {/* Glow on hover */}
                <div className="absolute inset-0 bg-gradient-to-t from-purple-500/30 via-pink-500/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl blur-xl" />
